        ...
        if (page.module) {
            ...
            $.each(page.module.topics, function (i) {
                ...
                if (topicComplete) {
                    ...
                    if ('menu' === page.type) {
                        $('.topic-mnu-item').eq(i).find('button').addClass('complete');
                    }
                }
                else if (rebus.config.takeTopicsInOrder && 'menu' === page.type && !previousTopicComplete) {
                    $('.topic-mnu-item').eq(i).find('button:not([data-always-unlocked])').prop('disabled', true);
                }
                if(!rebus.config.takeTopicsInOrder){
...
var navigator = (function () {
    var getIdOfPagePath = function (relativeToPage, path) {
        // console.log("getIdOfPagePath",relativeToPage ,path);

...
export default {
    init: function (partial) {
        $('[data-activity="carousel-select"]').each(function () {
            ...
            // Submit button
            if (submitType !== 'instant') {
                $('> div:last-of-type', $activity).append('<button type="button" class="btn btn-primary btn-submit-answer" disabled="disabled"><span>' + ($activity.data('submit-text') || 'Submit') + '</span></button>');
            }
            // Items

...
var showFinalPanel = function () {
    ...
    $questionPanels.each(function ( idx ) {
        ...
        var $activity = $(this).find('[data-activity]'),
            ...
            //console.log('question no.'+idx+ ' is a '+$type+' type')
            switch ( $type ){
    ...
    if (correctAmount === noOfQuestions) {
        ...
        if (page.type && page.type === 'pre-assessment') {
            activatePanel($('.pre-assessment-failed'));
        }else{
...
export default {
    ...
    init: function (partial) {
        ...
        $body.find('.btn-check-multi-choice-answer')
        .removeClass('btn-check-multi-choice-answer')
        ...
        if (!partial) {
            $body.on('click', '.btn-submit-assessment-answer', function () {
                ...
                if (page.type && page.type === 'post-assessment') {
                    var modalSelector = '#' + $activity.attr('data-storeid') + '-modal-' + (isCorrect ? 'correct' : 'incorrect');
                    //set backdrop on modal to static so we can't click out
                    $(modalSelector).attr("data-backdrop","static");
                    //open the modal
                    $(modalSelector).modal();
                    $('.modal.correct').on('click', '[data-dismiss="modal"]', function () {
